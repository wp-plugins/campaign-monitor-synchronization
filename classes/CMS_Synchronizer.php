		...
		if ( get_option( 'cms_update' ) ) {
			...
			if ( is_array( $result->response ) ) {
				foreach( $result->response as $key => $field ) {
					if( ! in_array( $field->Key, $cms_user_fields ) || in_array( $field->Key, $cms_fields_to_hide ) ) {
					...
					if ( in_array( $field->Key, $missing_fields ) ) {
						unset( $missing_fields[ array_search( $field->Key, $missing_fields ) ] );
					}
				}
			}
			...
			$result = $wrap_l->get_active_subscribers();
			if ( ! $result->was_successful() ) {
				self::$error = $result->response;
				return false;
			}
				...
				foreach( $result->response->Results as $key => $subscriber ) {
					...
					if ( ! $user ) {
						...
						foreach( $subscriber->CustomFields as $field ) {
							$custom_values[ $field->Key ] = $field->Value;
						}
