...
require_once CMS_PLUGIN_PATH . 'campaignmonitor-createsend-php/csrest_campaigns.php';
...
function cms_plugin_menu() {
	if ( basename( $_SERVER['SCRIPT_FILENAME'] ) == 'plugins.php' && isset( $_GET['page'] ) && $_GET['page'] == 'campaighmonitor-sync' ) {
		// Check permissions
		...
		if ( $_SERVER[ 'REQUEST_METHOD' ] == 'POST' ) {
			...
			if ( $result ) {
				wp_redirect( home_url( '/wp-admin/plugins.php?page=campaighmonitor-sync&saved=true' ) );
			} else {
				wp_redirect( home_url( '/wp-admin/plugins.php?page=campaighmonitor-sync&error=' . urlencode( CMS_Synchronizer::$error->Message ) ) );
			}
		}
	}
	
	add_plugins_page( 'Campaign Monitor Synchronization Options', 'Campaign Monitor Synch', 'manage_options', 'campaighmonitor-sync', 'cms_plugin_page' );
}
...
function cms_plugin_page() {
	...
	if ( isset( $_REQUEST['saved'] ) )
		echo '<div id="message" class="updated fade"><p><strong> ' . __( 'Settings saved.', 'campaighmonitor-sync' ) . '</strong></p></div>';
	if ( isset( $_REQUEST['error'] ) )
		echo '<div id="message" class="updated fade"><p><strong> ' . __( 'Campaign Monitor synchronization error.', 'campaighmonitor-sync' ) . '<br />' . __( urldecode( $_REQUEST['error'] ), 'campaighmonitor-sync' ) . '</strong></p></div>';
	?>
	<div class="wrap">
		...
		<form method="post">
			<h2><?php _e( 'Campaign Monitor Synchronization Options', 'campaighmonitor-sync' ); ?></h2>
			<div class="inside">
				<table border="0">
					<tbody>
						<tr>
							<td colspan="2"><h3><?php _e( 'General', 'campaighmonitor-sync' );?></h3></td>
						</tr>
						<tr>
							<td style="vertical-align: top;"><?php _e( 'User fields', 'campaighmonitor-sync' ); ?>:</td>
							<td>
								<?php
						...
						<tr>
							<td colspan="2"><h3><?php _e( 'API', 'campaighmonitor-sync' );?></h3></td>
						</tr>
						<tr>
							<td style="vertical-align: top;"><label for="cms_api_key"><?php _e( 'API key', 'campaighmonitor-sync' ); ?>:</label></td>
							<td><input type="text" name="cms_api_key" id="cms_api_key" value="<?php echo esc_attr( get_option( 'cms_api_key' ) ); ?>" size="70" /></td>
						</tr>
						<tr>
							<td style="vertical-align: top;"><label for="cms_list_id"><?php _e( 'List ID', 'campaighmonitor-sync' ); ?>:</label></td>
...
function cms_load_translation_file() {
	load_plugin_textdomain( 'campaighmonitor-sync', '', CMS_PLUGIN_PATH . 'translations' );
}
